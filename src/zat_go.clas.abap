...
public section.
  ...
  types:
    BEGIN OF msty_rule_detail .
        INCLUDE TYPE zatt_rule_detail.
    TYPES: routine_no TYPE progname.
    TYPES:  END OF msty_rule_detail .
  types:
    mtty_rule_detail TYPE TABLE OF msty_rule_detail .
  types:
    mtty_rule_code TYPE TABLE OF zatt_rule_code .
  types MSTY_CODE type STRING .
  types:
    mtty_code      TYPE STANDARD TABLE OF msty_code .

  data RULE type ZATT_RULE .
  data RULE_DETAIL type MTTY_RULE_DETAIL .
  data PARAMS type BGRFC_FUNINT_T .
  data VARS type MTTY_BAPI_VARIABLES .
  data STEP type ZATT_ITEM_STEP .
  data HEAD type ZATT_HEAD .
  data ITEM type ZATT_T_ITEM .
  data L_ITEM type ZATT_ITEM .
  data ITEM_STEP type ZATT_T_ITEM_STEP .
  data RET type MSTY_BAPI_RET .
  data RULE_CODE type MTTY_RULE_CODE .
  data PROGNAME type PROGNAME .
  class-data ABAP_TEMPLATE type PROGNAME value 'ZAT_RULE_ROUTINE_TEMPLATE' ##NO_TEXT.

  class-methods GO
    importing
      value(STEP) type ZATT_ITEM_STEP
      value(HEAD) type ZATT_HEAD
      value(ITEM) type ZATT_T_ITEM
      value(ITEM_STEP) type ZATT_T_ITEM_STEP
    returning
      value(R_OBJ) type ref to ZAT_GO .
  methods CONSTRUCTOR
    importing
      value(STEP) type ZATT_ITEM_STEP
      value(HEAD) type ZATT_HEAD
  ...
  class-methods CALL_TRANSACTION
    importing
      value(TYPE) type ANY
      value(DOCNR) type ANY
      value(CJAHR) type ANY optional .
...
private section.
  ...
  methods DEFINE_VAR
    importing
      value(PARAMS) type RFC_FUNINT
    changing
      value(VAR) type MSTY_BAPI_VARIABLE .
  methods TRANS_DATA
    exceptions
      ERROR .
  methods MOVE_VALUE
  ...
  methods SET_MOVE_VALUE_ROUTINE .
  methods GET_SOURCE
    importing
...
CLASS ZAT_GO IMPLEMENTATION.


  METHOD add_gos_relationship.
    CHECK me->ret-docnr IS NOT INITIAL.
    DATA: borident1 TYPE borident.
    DATA: borident2 TYPE borident.

    borident1-objkey = me->ret-docnr.
    borident1-objtype = rule-objtype.
    borident2-objkey = head-atno.
    borident2-objtype = 'ZAT'.
  ...
  METHOD constructor.
    ...
    INCLUDE zat_macro."引入返回的宏

    SELECT SINGLE * FROM zatt_rule INTO rule
      WHERE rule_name = step-rule_name."获取配置规则
    return_error 004.

    SELECT * FROM zatt_rule_detail INTO TABLE rule_detail
      WHERE rule_name = step-rule_name."获取配置规则明细
    return_error 004.

    SELECT * FROM zatt_rule_code INTO TABLE rule_code
       WHERE rule_name = step-rule_name."代码例程
    set_move_value_routine( ).
    ...
    me->head = head.
    me->item = item.
    me->item_step = item_step.
    me->ret-objtype = rule-objtype.

    params = zat_go=>get_function_interface( rule-bapi_name ).
    return_error 004.
  ...
  METHOD define_var.
    ...
    ENDCASE.
    var-bapi_kind = cl_abap_typedescr=>describe_by_data( var-bapi_variable->* )->type_kind.
  ENDMETHOD.
  ...
  METHOD move_value.
    CASE  is_rule-from_fieldalv.
      ...
      WHEN 'LAST'.
        ...
        IF is_rule-from_fname IS INITIAL.
          DATA(l_step) = step-step - 1.
        ELSE.
          l_step = is_rule-from_fname.
        ENDIF.
        READ TABLE item_step INTO DATA(ls_item_step) WITH KEY
              step = l_step.
        return_error 006.
        value = ls_item_step-docnr.
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD trans_return.
    ...
    IF rule-return_type = 'MB'.
      ...
      IF <g_mblnr> IS ASSIGNED AND <g_mjahr> IS ASSIGNED .
        me->ret-docnr = <g_mblnr>.
        me->ret-cjahr = <g_mjahr>.
      ENDIF.
    ENDIF.
  ...
  METHOD call_transaction.
    CHECK docnr IS NOT INITIAL.
    CASE type.
      WHEN 'EBAN' OR 'BUS2009'.
        SET PARAMETER ID 'BAN' FIELD docnr.
        CALL TRANSACTION 'ME53N' AND SKIP FIRST SCREEN.
      WHEN 'PO_CRE' OR 'BUS2012'.
      ...
      WHEN 'VL_GI' OR 'MB' OR 'BUS2017'.
        CALL FUNCTION 'MIGO_DIALOG'
          EXPORTING
            ...
            i_mjahr             = cjahr.
      WHEN 'SO_CRE' OR 'VBAK'.
        SET PARAMETER ID 'AUN' FIELD docnr.
        CALL TRANSACTION 'VA03' AND SKIP FIRST SCREEN.
      WHEN 'SO_VL_CRE' OR 'STO_VL_CRE' OR 'LIKP'.
        SET PARAMETER ID 'VL' FIELD docnr.
        CALL TRANSACTION 'VL03N' AND SKIP FIRST SCREEN.
      WHEN 'VBRK'.
        SET PARAMETER ID 'VF' FIELD docnr.
