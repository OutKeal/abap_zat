    ...
    FORM frm_abap_check  CHANGING error_flag.
      ...
      TRY.
          READ REPORT zat_go=>abap_template INTO source.
          subrc = sy-subrc.
    ...
    FORM frm_abap_save.
      ...
      LOOP AT g_code INTO DATA(l_code).
        APPEND INITIAL LINE TO gt_code ASSIGNING FIELD-SYMBOL(<fs_code>).
        <fs_code>-rule_name = gs_rule-rule_name.
        <fs_code>-action = g_action.
        <fs_code>-to_tabname = gs_func-parent.
        <fs_code>-to_fieldname = gs_func-fieldname.
        <fs_code>-line = sy-tabix.
        <fs_code>-code = l_code.
      ENDLOOP.
      LEAVE TO SCREEN 0.
    ENDFORM.
