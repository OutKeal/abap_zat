FUNCTION ZAT_DOMA_VALUE_GET.
*"----------------------------------------------------------------------
*"*"本地接口：
*"  IMPORTING
*"     REFERENCE(SPRAS) TYPE  SPRAS DEFAULT '1'
*"  TABLES
*"      DD04VV STRUCTURE  DD04VV
*"      DD04T STRUCTURE  DD04T OPTIONAL
*"      DOMA_VALUE STRUCTURE  ZATS_DOMA_VALUE
*"----------------------------------------------------------------------
...
*      l_table(10) TYPE c,
  ...
  IF dd04vv_def[] IS NOT INITIAL.                   "有固定域值的情况下
    ...
    IF sy-subrc EQ 0.
      LOOP AT dd07t.
        dd04vv_def = dd04vv_def[ domname = dd07t-domname ].
        IF sy-subrc EQ 0.
...
*创建动态工作区
      ...
      IF sy-subrc EQ 0.
        SELECT SINGLE * INTO @DATA(ls_dd03l)
