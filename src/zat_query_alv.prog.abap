...
MODULE status_0100 OUTPUT.
  ...
  CLEAR lt_excu[].
  IF p_read = 'X'.
    APPEND 'POST' TO lt_excu.
    APPEND 'CANCEL' TO lt_excu.
  ENDIF.

  SET PF-STATUS '100' EXCLUDING lt_excu.
ENDMODULE.
...
*    CALL METHOD cl_gui_cfw=>flush.
    ...
    ELSEIF gv_m = 'C'.
      PERFORM f_display_grid_alv_down USING gt_step_dis[].
    ENDIF.
  ELSE.
**--
...
FORM f_set_grid_field_catalog_down .
  ...
  IF gv_m = 'A'.
    ls_tabname = 'ZATS_ITEM'.
  ELSEIF gv_m = 'B'.
    ls_tabname = 'ZATS_MESSAGE'.
  ELSEIF gv_m = 'C'.
    ls_tabname = 'ZATS_ITEM_STEP'.
  ENDIF.
...
FORM f_handle_double_click  USING    pv_row TYPE lvc_s_row
  ...
  READ TABLE gt_head ASSIGNING <gs_head> INDEX pv_row-index .
  IF sy-subrc EQ  0.
