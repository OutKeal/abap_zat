...
FORM frm_get_single_data.
  IF gs_rule-rule_name IS INITIAL.
    ...
    STOP.
  ENDIF.
  SELECT  * FROM zatt_rule_detail INTO TABLE gt_detail WHERE rule_name = gs_rule-rule_name.
  SELECT  * FROM zatt_rule_code INTO TABLE gt_code WHERE rule_name = gs_rule-rule_name.
...
FORM frm_get_gt_func.
  ...
  DATA(lt_func) = zat_go=>get_function_interface( gs_rule-bapi_name ).
  IF sy-subrc <> 0.
    RETURN.
  ENDIF.
...
FORM frm_get_at_tree.
  PERFORM frm_get_fields_dfies USING 'ZATT_HEAD' '' 'HEAD' '抬头' cl_abap_typedescr=>typekind_struct1 CHANGING gt_at.
  PERFORM frm_get_fields_dfies USING 'ZATT_ITEM' '' 'ITEM' '项目' cl_abap_typedescr=>typekind_table CHANGING gt_at.
...
FORM frm_init_treel .
  IF g_tree_left IS INITIAL.
    ...
    DATA(it_fcat) =  zat_go=>fcat_from_itab( gt_tree_at ).
...
FORM frm_init_treer .
  IF  g_tree_right IS INITIAL.
    ...
    DATA(it_fcat) =  zat_go=>fcat_from_itab( gt_tree_func ).
    READ TABLE it_fcat ASSIGNING FIELD-SYMBOL(<fs_fcat>) WITH KEY fieldname = 'ICON'.
    IF sy-subrc EQ 0.
      <fs_fcat>-icon = 'X'.
    ENDIF.
...
FORM frm_save.
  ...
  DELETE FROM zatt_rule_detail WHERE rule_name = gs_rule-rule_name.
  DELETE FROM zatt_rule_code WHERE rule_name = gs_rule-rule_name.
  COMMIT WORK AND WAIT.
  MODIFY zatt_rule_detail FROM TABLE gt_detail.
  MODIFY zatt_rule_code FROM TABLE gt_code.
  COMMIT WORK AND WAIT.
  g_falv->soft_refresh( ).
ENDFORM.
